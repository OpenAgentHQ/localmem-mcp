# Python API reference

Generated from the source, so it never drifts from the code.

For task-oriented examples, see the [Python library guide](../guide/python-library.md).

---

## Memory store

::: localmem_mcp.store.MemoryStore

---

## Data types

::: localmem_mcp.store.Memory

::: localmem_mcp.store.SearchResult

---

## Embedders

::: localmem_mcp.store.Embedder

::: localmem_mcp.store.FastEmbedEmbedder

---

## Helpers

::: localmem_mcp.store.default_db_path

---

## Server

The MCP tool functions are documented in the [MCP tools guide](../guide/mcp-tools.md).
These are the module-level helpers for embedding the server in your own process.

::: localmem_mcp.server.get_store

::: localmem_mcp.server.configure
